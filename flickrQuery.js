...
function handleAddQuery (params, response) {
        // get URL and tag string from input params
    var url = params[0].split("=")[1];
    var tags = params[1].split("=")[1];

    // start database operation
    cmd = "INSERT INTO photos (fileName, tags) VALUES ('"+url+"', '"+tags+"')";
    db.run(cmd, dbCallback);

    function dbCallback(err) {
	sendResponseToBrowser(err,{}, response);
    }
}
